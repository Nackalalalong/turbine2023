...
def resconstruct(
    ...
    def iterate_exp_combinations(
        tensorbaord_save_dir: str,
        data: str = 'all'
        ):
    ...
    for data, data_dir, model_name, model_dir, ht_name, ht_dir in iterate_exp_combinations(tensorbaord_save_dir, data='1d'):
            ...
            for T in T_LIST:
                ...
                for H_index, H in enumerate(H_LIST):
                    ...
                    for i,feature in enumerate(['velocity', 'thrust', 'torqe']):
                        axes[i].plot(range(T), mses[:,i], label=str(H), color=colors[n_H - H_index  - 1], linewidth=3, alpha=0.75)
            ...
            for T_index, T in enumerate(T_LIST):
                ...
                for H_index, H in enumerate(H_LIST):
                    ...
                    for i,feature in enumerate(['velocity', 'thrust', 'torqe']):
                        ...
                        axes[T_index, i].plot(range(T), mses[:,i], label=str(H), color=colors[n_H - H_index  - 1], linewidth=3, alpha=0.75)
                        if T_index == 0:
