...
class ResidualBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        h = F.relu(
            self.linear_1(x))  # [B,L,in_dim] -> [B,L,hidden_dim] or [B,in_dim] -> [B,hidden_dim]
        h = self.dropout(
            self.linear_2(h))  # [B,L,hidden_dim] -> [B,L,out_dim] or [B,hidden_dim] -> [B,out_dim]
...
class Encoder(nn.Module):
    ...
    def __init__(self, layer_num, hidden_dim, seq_len, pred_len, dropout_rate, has_attribute):
        ...
        if has_attribute:
            attr_dim = 1
        self.first_encoder_layer = ResidualBlock(seq_len + attr_dim, hidden_dim, hidden_dim,
                                                 dropout_rate)
        self.other_encoder_layers = nn.ModuleList([
            ResidualBlock(hidden_dim, hidden_dim, hidden_dim, dropout_rate)
...
class Decoder(nn.Module):
    ...
    def __init__(self, layer_num, hidden_dim, pred_len, decoder_output_dim, temporal_decoder_hidden,
                 dropout_rate):
        super(Decoder, self).__init__()
        self.decoder_layer_num = layer_num
        self.horizon = pred_len
        self.last_decoder_layer = ResidualBlock(hidden_dim, hidden_dim,
                                                decoder_output_dim * pred_len, dropout_rate)
        self.other_decoder_layers = nn.ModuleList([
            ResidualBlock(hidden_dim, hidden_dim, hidden_dim, dropout_rate)
            for _ in range(layer_num - 1)
        ])
        self.temporaldecoder = ResidualBlock(decoder_output_dim, temporal_decoder_hidden, 1,
                                             dropout_rate)
...
class TiDE(ModelBehavior):
    ...
    def __init__(self, config: TiDEConfig):
        ...
        self.encoder = Encoder(encoder_layer_num, hidden_dim, seq_len, pred_len, dropout_rate,
                               has_attribute)
        self.decoder = Decoder(decoder_layer_num, hidden_dim, pred_len, decoder_output_dim,
